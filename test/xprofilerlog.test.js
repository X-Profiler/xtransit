...
describe('get xprofiler logs', function() {
  ...
  it('should get empty xprofiler logs with incorrect line', async function() {
    await writeFile(xprofilerLog, '[2020-06-04 00:32:05] [info2] [cpu] [89159] [1.2.1] cpu_usage(%) cpu_now: 0.000000, cpu_15: 0.000000, cpu_30: 0.000000, cpu_60: 0.042544\n', { flag: 'a' });
    await writeFile(xprofilerLog, '[2020-06-04 00:32:05] [info] [cpu2] [89159] [1.2.1] cpu_usage(%) cpu_now: 0.000000, cpu_15: 0.000000, cpu_30: 0.000000, cpu_60: 0.042544\n', { flag: 'a' });
    const message = await xprofilerLogLib.call(context);
    expect(message.type).to.be('xprofiler_log');
    expect(message.data.logs.length).to.be(0);
  });
  ...
  it('should get xprofiler logs', async function() {
    await writeFile(xprofilerLog, '[2020-06-04 00:32:05] [info] [cpu] [89159] [1.2.1] cpu_usage(%) cpu_now: 0.000000, cpu_15: 0.000000, cpu_30: 0.000000, cpu_60: 0.042544\n', { flag: 'a' });
    const message = await xprofilerLogLib.call(context);
    expect(message.type).to.be('xprofiler_log');
    expect(message.data.xprofiler_version).to.be('1.2.1');
